...
class FrictionRobotWrapper(RobotWrapper):
    ...
    :ivar np.ndarray fv: An array containing the viscous friction gain.
    ...
    def set_coulomb_friction_offset_gains(self, f0:(np.ndarray[float] or np.ndarray[np.ndarray[float]])):
        ...
        if f0.ndim == 1 and f0.size == self.model.nq:
            self.f0 = f0
        elif f0.ndim == 2 and (f0.shape[0] == 1 or f0.shape[1] == 1):
            self.f0 = f0.squeeze()
        else:
            raise ValueError(f"f0 must be a 1D array of size {self.model.nq}, according to the number of joint of the robot.")
